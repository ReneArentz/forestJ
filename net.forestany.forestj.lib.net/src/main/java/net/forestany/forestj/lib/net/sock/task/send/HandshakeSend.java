 ...
 */
public class HandshakeSend<T> extends net.forestany.forestj.lib.net.sock.task.Task<T> {
	/**
	 * interface delegate definition which can be instanced outside of net.forestany.forestj.lib.net.sock.task.send.HandshakeSend&lt;T&gt; class to execute code after triple handshake
	 */
	 ...
	 */
	public HandshakeSend(net.forestany.forestj.lib.net.sock.Type p_e_type) {
		super(p_e_type);
	}
	...
	public Class<?> getSocketTaskClassType() {
		return net.forestany.forestj.lib.net.sock.task.send.HandshakeSend.class;
	}
	...
	public void cloneFromOtherTask(net.forestany.forestj.lib.net.sock.task.Task<T> p_o_sourceTask) {
		...
		try { this.b_endless = ((net.forestany.forestj.lib.net.sock.task.send.HandshakeSend<T>)p_o_sourceTask).getEndless(); } catch (Exception o_exc) { /* NOP */ }
		try { this.i_taskIntervalMilliseconds = ((net.forestany.forestj.lib.net.sock.task.send.HandshakeSend<T>)p_o_sourceTask).getTaskIntervalMilliseconds(); } catch (Exception o_exc) { /* NOP */ }
		try { this.itf_delegateAdditionalExecution = ((net.forestany.forestj.lib.net.sock.task.send.HandshakeSend<T>)p_o_sourceTask).getDelegateAdditionalExecution(); } catch (Exception o_exc) { /* NOP */ }
	}
	
	/**
	 * runTask method of sending a handshake.
	 * 
	 * @throws Exception	any exception of implementation that could happen will be caught by abstract Task class, see details in protocol methods in net.forestany.forestj.lib.net.sock.task.send.HandshakeSend
	 */
	...
	private void handshakeTask() throws Exception {
		...
		if (this.e_type == net.forestany.forestj.lib.net.sock.Type.TCP) {
			...
			byte[] a_handshakeBytes = new byte[] {net.forestany.forestj.lib.net.sock.task.Task.BY_ACK_BYTE, (byte)net.forestany.forestj.lib.Helper.randomIntegerRange(0, 255)};
			...
			do {
				try {
					...
					if (i_attempts >= i_maxAttempts) { /* all attempts failed, so protocol for receiving handshake failed completely */
						throw new Exception("Handshake failed, sender first step, after " + i_attempts + " attempts");
					}
				}
			...
			do {
				...
				if (a_handshakeBytes[0] != net.forestany.forestj.lib.net.sock.task.Task.BY_ACK_BYTE) {
															net.forestany.forestj.lib.Global.ilogWarning("Invalid first byte of handshake[" + net.forestany.forestj.lib.Helper.printByteArray(new byte[] {a_handshakeBytes[0]}, false).trim() + "], must be [11111111]; retry after 25 milliseconds");
