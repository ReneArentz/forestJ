...
public class YAML {
	 ...
	 * set root element
	 * 
	 * @param p_o_value YAMLElement instance
	 */
	...
	/**
	 * get amount white spaces indentation
	 * 
	 * @return int
	 */
	...
	/**
	 * set amount white spaces indentation
	 * 
	 * @param p_i_amountWhiteSpacesindentation		amount of white spaces for indentation
	 * @throws IllegalArgumentException				must be at least '2'
	 */
	public void setAmountWhiteSpacesindentation(int p_i_amountWhiteSpacesindentation) throws IllegalArgumentException {
		...
		this.i_amountWhiteSpacesindentation = p_i_amountWhiteSpacesindentation;
												...
												net.forestany.forestj.lib.Global.ilogConfig("updates amount of white spaces indentation to '" + this.i_amountWhiteSpacesindentation + "'");
	}
	
	/**
	 * get string quote character
	 * 
	 * @return String
	 */
	 ...
	 * 
	 * @param p_s_stringQuote					character to quote strings within yaml
	 * @throws IllegalArgumentException			must be '1' character long
	 */
	public void setStringQuote(String p_s_stringQuote) throws IllegalArgumentException {
		...
		this.s_stringQuote = p_s_stringQuote;
												...
												net.forestany.forestj.lib.Global.ilogConfig("updates string quote to '" + this.s_stringQuote + "'");
	}
	...
	public YAML(java.util.List<String> p_a_yamlSchemaFileLines, int p_i_amountWhiteSpacesindentation, String p_s_stringQuote) throws IllegalArgumentException, NullPointerException {
	    ...
	    this.parseYAML(1, a_fileLines.size(), a_fileLines, 0, this.o_root, true);
	    										...
	    										net.forestany.forestj.lib.Global.ilogConfig("yaml schema parsed");
	...
	private void setSchema(boolean p_b_rootIsUnparsedSchema) {
		...
		if (p_b_rootIsUnparsedSchema) {
													if (net.forestany.forestj.lib.Global.isILevel(net.forestany.forestj.lib.Global.MASS)) net.forestany.forestj.lib.Global.ilogMass("Unparsed YAML-Schema: " + this.s_lineBreak + this.o_root);
			...
			this.parseYAMLSchemaElements(this.o_root);
													...
													net.forestany.forestj.lib.Global.ilogConfig("yaml schema elements parsed");
				...
				if (this.o_properties.getChildren().size() > 0) {
					...
					for (YAMLElement o_yamlElement : this.o_properties.getChildren()) {
						this.o_root.getChildren().add(o_yamlElement);
															...
															net.forestany.forestj.lib.Global.ilogConfig("added all 'properties' from yaml schema to root");
					...
					this.o_root.setReference(this.o_properties.getReference());
															...
															net.forestany.forestj.lib.Global.ilogConfig("set 'reference' from yaml schema as root 'reference'");
				}
			}
		}
		
												if (net.forestany.forestj.lib.Global.isILevel(net.forestany.forestj.lib.Global.MASS)) net.forestany.forestj.lib.Global.ilogMass("Parsed YAML-Schema: " + this.s_lineBreak + this.o_root);
		
		this.o_schema = this.o_root;
		
												net.forestany.forestj.lib.Global.ilogConfig("set yaml root element as schema element");
	}
	...
	public java.util.List<String> validateYAML(String p_s_yamlFile) throws java.io.IOException, IllegalArgumentException {
		...
		if (!net.forestany.forestj.lib.io.File.exists(p_s_yamlFile)) {
			throw new IllegalArgumentException("File[" + p_s_yamlFile + "] does not exist.");
		}
		
		/* open yaml file */
		net.forestany.forestj.lib.io.File o_file = new net.forestany.forestj.lib.io.File(p_s_yamlFile, false);
		
		/* load file content into string */
		String s_fileContent = o_file.getFileContent();
		
												net.forestany.forestj.lib.Global.ilogConfig("yaml file accessed and content read");
		...
		for (String s_line : s_fileContent.split(this.s_lineBreak)) {
			a_fileLines.add(s_line);
												...
												net.forestany.forestj.lib.Global.ilogConfig("yaml file content transferred to array");
		...
		for (int i_min = p_i_min; i_min < p_i_max; i_min++) {
			...
			if (s_line.charAt(p_i_whiteSpaceindentation) == ' ') {
				...
				this.i_level++;
														...
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "block goes from (" + (i_min + 1) + ") to (" + (i_max) + ") with indentation '" + i + "'");
			...
			if ( (!s_value.contentEquals("null")) && (!s_value.contentEquals(this.s_stringQuote + this.s_stringQuote)) && (s_value.length() > 0) ) {
				...
				if ( (s_value.startsWith(this.s_stringQuote)) && (s_value.endsWith(this.s_stringQuote)) ) {
					...
					if (!( (net.forestany.forestj.lib.Helper.isInteger(s_value)) || (net.forestany.forestj.lib.Helper.isDouble(s_value)) )) { /* leading '-' is valid if value is integer or double */
				...
				s_value = "null";
													...
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "(" + (i_min + 1) + ")|key=" + s_key + "|value=" + s_value + "|" + b_newElementForCollection + "|" + p_b_textBlock + "|");
		}
	}
	...
	public YAMLValueType getYAMLValueType(String p_s_yamlValue) throws IllegalArgumentException {
		...
		/* get yaml value type */
		if (net.forestany.forestj.lib.Helper.isStringEmpty(p_s_yamlValue)) {
			e_yamlValueType = YAMLValueType.Null;
		} else if (net.forestany.forestj.lib.Helper.isDateTime(p_s_yamlValue)) {
	...
	public YAMLElement parseYAML(int p_i_min, int p_i_max, java.util.List<String> p_a_lines, int p_i_whiteSpaceindentation, YAMLElement p_o_yamlElement, boolean p_b_parseSchema) throws IllegalArgumentException, NullPointerException {
		...
		for (int i_min = p_i_min; i_min < p_i_max; i_min++) {
			...
			if (s_line.charAt(p_i_whiteSpaceindentation) == ' ') {
				...
				if (s_reference != null) {
															net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "store reference: " + s_reference);
			...
			if ( (b_newElementForCollection) && (!p_b_parseSchema) ) {
				if ( (!b_collectionLevelFlag) || (!s_key.contentEquals("null")) ) {
					if (!s_key.contentEquals("null")) {
						if (o_yamlOldCurrentLevelElement != null) {
							...
							this.i_level--;
																	...
																	net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "array object before is finished after new element for collection appeared and array level flag is still 'true'");
						}
					}
						
					b_collectionLevelFlag = true;
					
															net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "new array object");
					...
					this.i_level++;
													...
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "(" + (i_min + 1) + ")|key=" + s_key + "|value=" + s_value + "|" + b_newElementForCollection + "|");
			...
			if (!s_value.contentEquals("null")) {
				o_yamlElement.setValue(s_value);
													...
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + o_yamlCurrentLevelElement.getName() + ".getChildren().add(" + o_yamlElement.getName() + ");");
			o_yamlCurrentLevelElement.getChildren().add(o_yamlElement);
			
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "p_o_yamlElement = " + o_yamlElement.getName() + ";");
		...
		if (p_o_yamlElement.getName().contentEquals(o_yamlCurrentLevelElement.getName())) {
			...
			return p_o_yamlElement;
													...
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "return " + o_yamlCurrentLevelElement.getName());
	...
	private void parseYAMLSchemaElements(YAMLElement p_o_yamlSchemaElement) throws IllegalArgumentException, NullPointerException {
		if (p_o_yamlSchemaElement.getChildren().size() > 0) {
			...
			for (YAMLElement o_yamlChild : p_o_yamlSchemaElement.getChildren()) {
				if (o_yamlChild.getName().toLowerCase().contentEquals("type")) {
					...
					b_items = true;
													...
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "element: '" + p_o_yamlSchemaElement.getName() + "' - array=" + b_array + "|object=" + b_object + "|properties=" + b_properties + "|items=" + b_items + "|");
			...
			for (YAMLElement o_yamlChild : p_o_yamlSchemaElement.getChildren()) {
				...
				if (net.forestany.forestj.lib.Helper.isStringEmpty(o_yamlChild.getValue())) {
					e_yamlValueType = YAMLValueType.Object;
				} else {
					...
					if ( (e_yamlValueType == YAMLValueType.String) && (o_yamlChild.getValue().charAt(0) == '&') ) {
						...
						if (s_referenceName == null) {
							...
							o_yamlChild.setValue(null);
														...
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + o_yamlChild.getName() + "(" + e_yamlValueType + ") of " + p_o_yamlSchemaElement.getName());
				...
				if (o_yamlChild.getLevel() == 0) {
					if ( (o_yamlChild.getName().toLowerCase().contentEquals("definitions")) || (o_yamlChild.getName().toLowerCase().contentEquals("properties")) ) {
						...
						if (e_yamlValueType == YAMLValueType.Object) {
																	net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "save current element(" + this.o_currentElement.getName() + ") in temporary variable");
							
							/* save current element in temporary variable */
							YAMLElement o_oldCurrentElement = this.o_currentElement;

																	net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "start recursion for: " + o_yamlChild.getName() + " = (" + e_yamlValueType + ") for " + this.o_currentElement.getName());
							...
							if (o_yamlChild.getName().toLowerCase().contentEquals("definitions")) {
								...
								this.o_properties = this.o_currentElement;
																	...
																	net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "end recursion for " + this.o_currentElement.getName() + ": " + o_yamlChild.getName() + " = (" + e_yamlValueType + ")");
							
																	net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "reset current element(" + this.o_currentElement.getName() + ") from temporary variable(" + o_oldCurrentElement.getName() + ")");
				...
				if (e_yamlValueType == YAMLValueType.Object) {
					...
					if (!b_skip) { /* new object */
																net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + o_yamlChild.getName() + " = (" + e_yamlValueType + ")");
						...
						if (o_yamlChild.getName().toLowerCase().contentEquals("properties")) {
							...
							this.parseYAMLSchemaElements(o_yamlChild);
																	...
																	net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "end recursion for: " + o_yamlChild.getName() + " = (" + e_yamlValueType + ")");
						} else {
																	net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "save current element(" + this.o_currentElement.getName() + ") in temporary variable");
							...
							if (b_handleRootItems) {
								...
								if (o_newYAMLElement.getChildren().size() == 1) {
																			net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "setReference for (" + this.o_currentElement.getName() + ") with object(" + o_newYAMLElement.getChildren().get(0).getName() + ")");
							...
							if (this.o_currentElement.getName().toLowerCase().contentEquals("definitions")) {
								this.o_definitions = this.o_currentElement;
															...
															net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + o_yamlChild.getName() + " = " + o_yamlChild.getValue() + " (" + e_yamlValueType + ") for " + this.o_currentElement.getName());
	...
	public File yamlEncode(Object p_o_object, String p_s_yamlFile, boolean p_b_overwrite) throws java.io.IOException, NullPointerException, IllegalArgumentException, NoSuchFieldException, NoSuchMethodException, java.lang.reflect.InvocationTargetException, IllegalAccessException, ClassNotFoundException {
		...
		String s_yaml = this.yamlEncode(p_o_object);
												...
												if (net.forestany.forestj.lib.Global.isILevel(net.forestany.forestj.lib.Global.MASS)) net.forestany.forestj.lib.Global.ilogMass("Encoded YAML:" + this.s_lineBreak + s_yaml);
		...
		/* open (new) file */
		net.forestany.forestj.lib.io.File o_file = new net.forestany.forestj.lib.io.File(p_s_yamlFile, !p_b_overwrite);
	...
	private String yamlEncodeRecursive(YAMLElement p_o_yamlSchemaElement, Object p_o_object, boolean p_b_parentIsCollection) throws NullPointerException, IllegalArgumentException, NoSuchFieldException, NoSuchMethodException, java.lang.reflect.InvocationTargetException, IllegalAccessException, ClassNotFoundException {
		...
		if (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlSchemaElement.getMappingClass())) {
		...
		if (p_o_yamlSchemaElement.getType().toLowerCase().contentEquals("object")) {
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "cast schema-object(" + p_o_yamlSchemaElement.getName() + ") with schema-mapping(" + p_o_yamlSchemaElement.printMapping() + ") and p_o_object(" + p_o_object.getClass().getTypeName() + "), castonly=" + (p_o_yamlSchemaElement.getMappingClass().contentEquals(p_o_object.getClass().getTypeName())));
			...
			if (o_object == null) {
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "casted schema-object(" + p_o_yamlSchemaElement.getName() + ") with schema-mapping(" + p_o_yamlSchemaElement.printMapping() + ") is null");
				...
				if ( (this.i_level == 0) || (p_b_parentIsCollection) ) {
					...
					if (!net.forestany.forestj.lib.Helper.isStringEmpty(s_yamlParentName)) {
					...
					if (p_o_yamlSchemaElement.getReference().getChildren().size() < 1) {
						...
						if (p_o_yamlSchemaElement.getReference().getReference() == null) {
							...
							b_childrenIteration = false;
																	...
																	net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-object(" + p_o_yamlSchemaElement.getName() + ") with its reference(" + p_o_yamlSchemaElement.getReference().getName() + ") and schema-mapping(" + p_o_yamlSchemaElement.printMapping() + ") which has another reference with recursion");
							
							/* handle reference with recursion */
							s_yaml += this.yamlEncodeRecursive(p_o_yamlSchemaElement.getReference(), o_object, false);
						}
					} else {
																net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-object(" + p_o_yamlSchemaElement.getName() + ") with its reference(" + p_o_yamlSchemaElement.getReference().getName() + ") which has children");
				...
				if (b_childrenIteration) {
					...
					for (YAMLElement o_yamlElement : p_o_yamlSchemaElement.getChildren()) {
																net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "encode schema-object's(" + p_o_yamlSchemaElement.getName() + ") child(" + o_yamlElement.getName() + ") with schema-mapping(" + p_o_yamlSchemaElement.printMapping() + ") with recursion");
			...
			if (p_o_yamlSchemaElement.getReference() != null) {
				if (p_o_yamlSchemaElement.getReference().getChildren().size() < 1) {
					throw new IllegalArgumentException("Reference[" + p_o_yamlSchemaElement.getReference().getName() + "] of schema-element[" + p_o_yamlSchemaElement.getName() + "] with schema-type[" + p_o_yamlSchemaElement.getType() + "] must have at least one child");
														...
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-array(" + p_o_yamlSchemaElement.getName() + ") with its reference(" + p_o_yamlSchemaElement.getReference().getName() + ") which has children");
			} else {
				if (p_o_yamlSchemaElement.getChildren().size() != 1) {
				...
				if (!p_o_yamlSchemaElement.getChildren().get(0).getName().toLowerCase().contentEquals("items")) {
					throw new IllegalArgumentException("Schema-element[" + p_o_yamlSchemaElement.getName() + "] with schema-type[" + p_o_yamlSchemaElement.getName() + "] must have one child with name[items]");
														...
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-array(" + p_o_yamlSchemaElement.getName() + ") with child(items) which has children");
			}
			
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-array(" + p_o_yamlSchemaElement.getName() + ") - and cast object with p_o_object(" + p_o_object.getClass().getTypeName() + ") and schema-mapping(" + p_o_yamlSchemaElement.printMapping() + "), castonly if java.util.List=" + (p_o_object instanceof java.util.List));
			...
			if ( (o_object == null) && (p_o_yamlSchemaElement.getChildren().get(0).getPrimitiveArray()) ) {
				...
				for (int i = 0; i < a_objects.size(); i++) {
					...
					if (a_objects.get(i) == null) {
																net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "encode schema-array(" + p_o_yamlSchemaElement.getName() + ") with array element(null) and schema-mapping(" + p_o_yamlSchemaElement.printMapping() + ")");
						...
						this.i_level--;
																...
																net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "encode schema-array(" + p_o_yamlSchemaElement.getName() + ") with primitive array element(#" + (i + 1) + ") and schema-mapping(" + p_o_yamlSchemaElement.printMapping() + ")");
						...
						this.i_level--;
																...
																net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "encode schema-array(" + p_o_yamlSchemaElement.getName() + ") with array element(" + a_objects.get(i).getClass().getTypeName() + ") and schema-mapping(" + p_o_yamlSchemaElement.printMapping() + ") with recursion");
			...
			Object o_object = p_o_object;
													...
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-property(" + p_o_yamlSchemaElement.getName() + ") with p_o_object(" + p_o_object.getClass().getTypeName() + ")");
			
			/* get object property if we have not an array with items */
			if (!p_o_yamlSchemaElement.getName().toLowerCase().contentEquals("items")) {
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-property(" + p_o_yamlSchemaElement.getName() + "), cast object with p_o_object(" + p_o_object.getClass().getTypeName() + ")");
				
				/* get object property of current yaml element */
				o_object = this.castObject(p_o_yamlSchemaElement, p_o_object);
			}
			
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-property(" + p_o_yamlSchemaElement.getName() + "), cast string from object with p_o_object(" + p_o_object.getClass().getTypeName() + ") and schema-type(" + p_o_yamlSchemaElement.getType() + ")");
	...
	private Object castObject(YAMLElement p_o_yamlElement, Object p_o_object, boolean b_castOnly) throws NoSuchFieldException, NoSuchMethodException, java.lang.reflect.InvocationTargetException, IllegalAccessException, ClassNotFoundException {
		...
		if (b_castOnly) {
			...
			if (!net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlElement.getMapping())) {
	...
	private void checkRestriction(String p_s_value, YAMLRestriction p_o_yamlRestriction, YAMLElement p_o_yamlElement) throws IllegalArgumentException {
		...
		if (p_o_yamlRestriction.getName().toLowerCase().contentEquals("minimum")) {
			...
			if ( (p_s_type.contentEquals("string")) || (p_s_type.contentEquals("boolean")) || (p_s_type.contentEquals("number")) || (p_s_type.contentEquals("integer")) )  {
				if (!net.forestany.forestj.lib.Helper.matchesRegex(p_s_value, p_o_yamlRestriction.getStrValue())) {
	...
	public boolean validateAgainstSchema(String p_s_yamlFile) throws IllegalArgumentException, java.io.IOException, NullPointerException, java.text.ParseException, java.time.DateTimeException {
		...
		for (String s_line : s_fileContent.split(this.s_lineBreak)) {
			a_fileLines.add(s_line);
												...
												net.forestany.forestj.lib.Global.ilogFinest("read all lines from yaml file '" + p_s_yamlFile + "'");
	...
	public boolean validateAgainstSchema(java.util.List<String> p_a_yamlLines) throws NullPointerException, IllegalArgumentException, java.text.ParseException, java.time.DateTimeException {
	    ...
	    this.parseYAML(1, a_fileLines.size(), a_fileLines, 0, this.o_root);
	    										...
	    										net.forestany.forestj.lib.Global.ilogFinest("parsed yaml content lines");
	    ...
	    this.o_currentElement = null;
											    ...
											    if (net.forestany.forestj.lib.Global.isILevel(net.forestany.forestj.lib.Global.MASS)) net.forestany.forestj.lib.Global.ilogMass("YAML-Schema:" + net.forestany.forestj.lib.io.File.NEWLINE + this.o_schema);
											    if (net.forestany.forestj.lib.Global.isILevel(net.forestany.forestj.lib.Global.MASS)) net.forestany.forestj.lib.Global.ilogMass("YAML-Root-Element:" + net.forestany.forestj.lib.io.File.NEWLINE + this.o_root);
	...
	private boolean validateAgainstSchemaRecursive(YAMLElement p_o_yamlDataElement, YAMLElement p_o_yamlSchemaElement) throws NullPointerException, IllegalArgumentException, java.text.ParseException, java.time.DateTimeException {
		...
		if (p_o_yamlSchemaElement.getType().toLowerCase().contentEquals("object")) {
			...
			if (p_o_yamlDataElement.getChildren().size() > 0) {
				...
				if ( (p_o_yamlSchemaElement.getReference() != null) && (p_o_yamlSchemaElement.getReference().getType().toLowerCase().contentEquals("object")) ) {
					s_objectType = p_o_yamlSchemaElement.getReference().getMappingClass();
														...
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_yamlSchemaElement.getName() + ": create new schema-object instance with mapping[" + p_o_yamlSchemaElement.printMapping() + "] and type[" + s_objectType + "]");
					...
					if (p_o_yamlSchemaElement.getReference().getChildren().size() < 1) {
						...
						if (p_o_yamlSchemaElement.getReference().getReference() == null) {
							...
							if (p_o_yamlSchemaElement.getName().contentEquals(p_o_yamlDataElement.getName())) {
																		net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_yamlSchemaElement.getName() + ": get schema-object[" + p_o_yamlSchemaElement.getMappingClass() + "] with reference[" + p_o_yamlSchemaElement.getReference().getName() + "]");
								...
								b_return = this.validateAgainstSchemaRecursive(p_o_yamlDataElement.getChildren().get(0), p_o_yamlSchemaElement.getReference());
																...
																net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "update current schema-element[" + p_o_yamlSchemaElement.getName() + "](" + p_o_yamlSchemaElement.printMapping() + ") with reference[" + p_o_yamlSchemaElement.getReference().getName() + "](" + p_o_yamlSchemaElement.getReference().printMapping() + ")");
				...
				/* execute children iteration */
				if (b_childrenIteration) {
															net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_yamlSchemaElement.getName() + ": get schema-object with type[" + p_o_yamlSchemaElement.getMappingClass() + "] and children[" + p_o_yamlSchemaElement.getChildren().size() + "]");
			...
			if (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlDataElement.getValue())) { /* we have multiple minor objects for current array */
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_yamlSchemaElement.getName() + ": get schema-array with mapping[" + p_o_yamlSchemaElement.printMapping() + "]");
				...
				if (p_o_yamlDataElement.getChildren().size() > 0) { /* if we have objects to the new array */
					...
					for (int i = 0; i < p_o_yamlDataElement.getChildren().size(); i++) {
						...
						b_return = this.validateAgainstSchemaRecursive(p_o_yamlDataElement.getChildren().get(i), p_o_yamlSchemaElement);
																...
																net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + " return value of " + p_o_yamlDataElement.getName() + ": " + b_return);
						...
						this.i_level--;
														...
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_yamlSchemaElement.getName() + ": handle array value = " + p_o_yamlDataElement.getValue() + " - mapping[" + p_o_yamlSchemaElement.printMapping() + "]");
				
				/* set array with values if we have any values */
				if ( (!net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlDataElement.getValue())) && (!p_o_yamlDataElement.getValue().contentEquals("[]")) ) {
					...
					for (String s_value : a_values) {
						...
						Object o_returnObject = this.castObjectFromString(s_value, p_o_yamlSchemaElement.getType());
																...
																net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "add value[" + ( (o_returnObject == null) ? "null" : o_returnObject ) + "] to list of " + p_o_yamlDataElement.getName() + ", type[" + ( (o_returnObject == null) ? "null" : o_returnObject.getClass().getTypeName() ) + "]");
					}
					...
					throw new IllegalArgumentException("'" + p_o_yamlSchemaElement.getName() + "' of '" + s_requiredProperty + "' is required, but array has no values");
													...
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_yamlSchemaElement.getName() + ": handle value = " + p_o_yamlDataElement.getValue() + " - mapping[" + p_o_yamlSchemaElement.printMapping() + "]");
			...
			if ( (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlSchemaElement.getMapping())) && (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlSchemaElement.getMappingClass())) ) {
				...
				try {
					...
					try {
						...
						Object o_foo = this.castObjectFromString(s_objectValue, p_o_yamlSchemaElement.getMappingClass());
												...
												net.forestany.forestj.lib.Global.ilogFine(this.printIndentation() + "return " + b_return);
		return b_return;
	}
	...
	private Object yamlDecodeRecursive(YAMLElement p_o_yamlDataElement, YAMLElement p_o_yamlSchemaElement, Object p_o_object) throws NullPointerException, IllegalArgumentException, NoSuchFieldException, NoSuchMethodException, java.lang.reflect.InvocationTargetException, IllegalAccessException, java.text.ParseException, java.time.DateTimeException, InstantiationException, ClassNotFoundException {
		...
		if (p_o_yamlSchemaElement.getType().toLowerCase().contentEquals("object")) {
			...
			if (p_o_yamlDataElement.getChildren().size() < 1) {
				...
				if (!net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlSchemaElement.getMapping())) {
				...
				if (p_o_yamlSchemaElement.getReference() != null) {
					...
					if (p_o_yamlSchemaElement.getReference().getChildren().size() < 1) {
						...
						if (p_o_yamlSchemaElement.getReference().getReference() == null) {
							...
							/* check if current element in schema has data element by name, otherwise skip this element */
							if (p_o_yamlSchemaElement.getName().contentEquals(p_o_yamlDataElement.getName())) {
																		net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_yamlSchemaElement.getName() + ": get schema-object[" + o_object.getClass().getTypeName() + "] with reference[" + p_o_yamlSchemaElement.getReference().getName() + "]");
				...
				/* execute children iteration */
				if (b_childrenIteration) {
															net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_yamlSchemaElement.getName() + ": get schema-object with type[" + o_object.getClass().getTypeName() + "] and children[" + p_o_yamlSchemaElement.getChildren().size() + "]");
					...
					if ( (p_o_object != null) && (!(p_o_object instanceof java.util.List)) ) {
																net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_yamlSchemaElement.getName() + ": set schema-object[" + o_object.getClass().getTypeName() + "] to current object[" + p_o_object.getClass().getTypeName() + "] with mapping[" + s_objectMapping + "]");
			...
			if ( (p_o_yamlSchemaElement.getChildren().size() > 0) && (p_o_yamlSchemaElement.getChildren().get(0).getPrimitiveArray()) ) {
				...
				if ( (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlSchemaElement.getMapping())) && (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlSchemaElement.getMappingClass())) ) {
					...
					} else if (!(net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlSchemaElement.getMappingClass()))) {
				...
				if (p_o_object == null) { /* create a new object instance of yaml array element */
					...
					o_objectList = (Object)(new java.util.ArrayList<Object>());
															...
															net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "p_o_object(" + p_o_object.getClass().getTypeName() + ") != null, get list property " + p_o_yamlSchemaElement.getName());
				...
				if (p_o_yamlDataElement.getChildren().size() > 0) { /* if we have objects to the new array */
					...
					for (int i = 0; i < p_o_yamlDataElement.getChildren().size(); i++) {
						...
						java.util.List<Object> o_temp = (java.util.List<Object>)o_objectList;
																...
																net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "add return value to list of " + p_o_yamlDataElement.getName() + ", type[" + ((o_returnObject == null) ? "object is null" : o_returnObject.getClass().getTypeName()) + "]");
			...
			if (b_primitiveArray) {
				...
				if (p_o_object == null) {
					throw new NullPointerException("YAML schema element[" + p_o_yamlSchemaElement.getName() + "] has not initiated object for primitive array[" + s_primitiveArrayMapping + "]");
														...
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "set primitive array for [" + s_primitiveArrayMapping + "] of object [" + p_o_object.getClass().getTypeName() + "]");
			...
			if (p_o_object == null) {
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "p_o_object == null, set p_o_object = o_objectList");
			...
			if ( (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlSchemaElement.getMapping())) && (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_yamlSchemaElement.getMappingClass())) ) {
				...
				this.setProperty(p_o_yamlSchemaElement, p_o_object, p_o_yamlDataElement.getValue());
												...
												net.forestany.forestj.lib.Global.ilogFine(this.printIndentation() + "return " + ((p_o_object == null) ? "null" : p_o_object.getClass().getTypeName()));
		return p_o_object;
	}
	...
	*/
		...
		if (this.b_usePropertyMethods) {
			...
			try {
				...
				o_method.invoke(p_o_object, this.castObjectFromString(p_s_objectValue, p_o_yamlSchemaElement.getType()));
														...
														net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + o_exc.getMessage() + " - with type[" + p_o_yamlSchemaElement.getType() + "] - try another cast with type[" + p_o_object.getClass().getDeclaredField(s_field).getType().getTypeName() + "]");
	...
	public enum YAMLValueType {
		/**
		 * String yaml value type
		 */
		String,
		/**
		 * Number yaml value type
		 */
		Number,
		/**
		 * Integer yaml value type
		 */
		Integer,
		/**
		 * Boolean yaml value type
		 */
		Boolean,
		/**
		 * Array yaml value type
		 */
		Array,
		/**
		 * Object yaml value type
		 */
		Object,
		/**
		 * Null yaml value type
		 */
		Null
	}
	...
	public class YAMLElement {
		 ...
		 * set description
		 * 
		 * @param p_s_value description value
		 */
		 ...
		 * set mapping class
		 * 
		 * @param p_s_value mapping class value
		 */
		 ...
		 * set requires flag
		 * 
		 * @param p_b_value true - element is required
		 */
		 ...
		 * set primitive array flag
		 * 
		 * @param p_b_value true - element holds a primitive array
		 */
		 ...
		 * set reference
		 * 
		 * @param p_o_value reference to other YAMLElement
		 */
		 ...
		 * 
		 * @return java.util.List&lt;YAMLElement&gt;
		 */
		...
		/**
		 * get restrictions of element
		 * 
		 * @return java.util.List&lt;YAMLRestriction&gt;
		 */
					...
					if (o_field.get(this) instanceof java.util.List<?>) {
						...
						if (a_objects.size() > 0) {
							...
							for (Object o_object : a_objects) {
								s_foo += o_object.toString() + net.forestany.forestj.lib.io.File.NEWLINE;
							}
						} else {
		 ...
		 * print mapping
		 * 
		 * @return mapping string of yaml element, mapping class or a combination of mapping and mapping class separated by ':'
		 */
		public String printMapping() {
			String s_foo = "";
			
			if ( (net.forestany.forestj.lib.Helper.isStringEmpty(this.getMapping())) && (!net.forestany.forestj.lib.Helper.isStringEmpty(this.getMappingClass())) ) {
				s_foo = this.getMappingClass();
			} else if ( (!net.forestany.forestj.lib.Helper.isStringEmpty(this.getMapping())) && (!net.forestany.forestj.lib.Helper.isStringEmpty(this.getMappingClass())) ) {
	...
	public class YAMLRestriction {
		...
		/**
		 * get string value of restriction
		 * 
		 * @return String
		 */
		...
		/**
		 * set string value of restriction
		 * 
		 * @param p_s_value string value of restriction
		 */
		...
		/**
		 * set int value of restriction
		 * 
		 * @param p_i_value int value of restriction
		 */
