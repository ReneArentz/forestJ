...
public class StateMachine {
	...
	/**
	 * State machine EXIT state constant
	 */
	...
	public void addTransition(String p_s_fromState, String p_s_returnCode, String p_s_toState) throws IllegalArgumentException {
    	...
    	if (!this.a_returnCodes.contains(p_s_returnCode)) {
    		throw new IllegalArgumentException("Invalid return code '" + p_s_returnCode + "', valid states are [" + net.forestany.forestj.lib.Helper.joinList(this.a_returnCodes, ',') + "]");
    	}
    	
    	if (!this.a_states.contains(p_s_toState)) {
    		throw new IllegalArgumentException("Invalid state '" + p_s_toState + "', valid states are [" + net.forestany.forestj.lib.Helper.joinList(this.a_states, ',') + "]");
    	}
	 ...
	 * @throws IllegalStateException	State machine is not ready, transition configuration must be loaded first
	 * @see net.forestany.forestj.lib.StateMachine.StateMethodContainer
	 */
	public void addStateMethod(StateMethodContainer o_stateMethodContainer) throws IllegalStateException {
		...
		/* invalid state found */
		if (!b_found) {
			throw new IllegalStateException("Invalid state '" + o_stateMethodContainer.getState() + "', valid states are [" + net.forestany.forestj.lib.Helper.joinList(this.a_states, ',') + "]");
		}
	...
	public class StateMethodContainer {
		...
		/**
		 * get state method interface
		 * @return StateMethodInterface
		 */
