...
public class Timer {
	...
	public void startTimer() {
	    ...
	    if (this.o_timerTask.getStartTime() != null) {
	    	/* start delay calculation, so we subtract current time from set start time */
	    											net.forestany.forestj.lib.Global.ilogConfig("Start delay calculation with start time '" + this.o_timerTask.getStartTime() + "'");
	    	...
	    	l_startDelay = this.o_timerTask.getStartTime().toNanoOfDay() / 1_000_000 - o_localTime.toNanoOfDay() / 1_000_000;
	    											...
	    											net.forestany.forestj.lib.Global.ilogConfig("Calculated start delay: '" + l_startDelay + " ms'");	    	
			/* if calculated start delay is negative, it will be set for next day */ 
			if (l_startDelay < 0) {
				l_startDelay += 24 * 60 * 60 * 1_000;
														net.forestany.forestj.lib.Global.ilogConfig("start delay is negative, it will be set for next day: '" + java.time.LocalDateTime.now().plusSeconds(l_startDelay / 1_000) + "'");
			}
	    }
	    ...
	    this.o_timer.scheduleAtFixedRate(this.o_timerTask, l_startDelay, this.o_timerTask.getInterval().toDuration());
	    										net.forestany.forestj.lib.Global.ilogFinest("timer started");
	}
	...
	public void stopTimer() {
		this.o_timer.cancel();
												net.forestany.forestj.lib.Global.ilogFinest("timer stopped");
	}
}
