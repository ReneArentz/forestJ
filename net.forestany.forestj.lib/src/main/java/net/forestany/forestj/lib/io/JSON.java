...
public class JSON {
	 ...
	 * set root element
	 * 
	 * @param p_o_value JSONElement instance
	 */
	...
	public JSON(java.util.List<String> p_a_jsonSchemaLines) throws IllegalArgumentException, NullPointerException {
	    ...
	    this.validateJSON(s_json);
	    										...
	    										net.forestany.forestj.lib.Global.ilogConfig("json schema file lines validated");
	    
	    /* parse json */
	    this.parseJSON(s_json);
	    
	    										net.forestany.forestj.lib.Global.ilogConfig("json schema parsed");
	...
	public JSON(String p_s_file) throws IllegalArgumentException, NullPointerException, java.io.IOException {
	    ...
	    this.validateJSON(s_json);
	    										...
	    										net.forestany.forestj.lib.Global.ilogConfig("json schema file validated");
	...
	private void setSchema(boolean p_b_rootIsUnparsedSchema) {
	    ...
	    if (p_b_rootIsUnparsedSchema) {
	    											if (net.forestany.forestj.lib.Global.isILevel(net.forestany.forestj.lib.Global.MASS)) net.forestany.forestj.lib.Global.ilogMass("Unparsed JSON-Schema: " + this.s_lineBreak + this.o_root);
		    ...
		    this.parseJSONSchema(this.o_root);
		    										...
		    										net.forestany.forestj.lib.Global.ilogConfig("json schema elements parsed");
		    ...
		    if (this.a_properties != null) {
		    	...
		    	if (this.a_properties.getChildren().size() > 0) {
		    		...
		    		this.o_root.setReference(this.a_properties.getReference());
	    										...
	    										if (net.forestany.forestj.lib.Global.isILevel(net.forestany.forestj.lib.Global.MASS)) net.forestany.forestj.lib.Global.ilogMass("Parsed JSON-Schema: " + this.s_lineBreak + this.o_root);
	    
	    this.o_schema = this.o_root;
	    
	   											net.forestany.forestj.lib.Global.ilogConfig("set json root element as schema element");
	}
		...
		while (p_s_json.charAt(i) != '}') {
			...
			while ( (p_s_json.charAt(i) != ',') || (b_doubleQuoteActive) || (i_level != 0) ) {
				...
				i++;
													...
													net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + s_jsonLine);
			...
			if ( (s_jsonLine.startsWith("{")) && (s_jsonLine.endsWith("}")) ) {
				...
				if ( (e_jsonValueType == JSONValueType.Object) || (b_handleArrayAsObject) ) {
															net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + s_jsonProperty + " = (" + e_jsonValueType + ")");
					/* save current element in temporary variable */
					JSONElement o_oldCurrentElement = this.o_currentElement;
					...
					this.o_currentElement = o_oldCurrentElement;
															...
															net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + s_jsonProperty + " = " + s_jsonValue + " (" + e_jsonValueType + ")");
	...
	private void parseJSONSchema(JSONElement p_o_jsonElement) throws IllegalArgumentException, NullPointerException {
		if (p_o_jsonElement.getChildren().size() > 0) {
			...
			for (JSONElement o_jsonChild : p_o_jsonElement.getChildren()) {
				...
				/* determine json value type */
				if (net.forestany.forestj.lib.Helper.isStringEmpty(o_jsonChild.getValue())) {
					e_jsonValueType = JSONValueType.Object;
				} else {
					...
					if ( (o_jsonChild.getValue().startsWith("\"")) && (o_jsonChild.getValue().endsWith("\"")) ) {
						o_jsonChild.setValue(o_jsonChild.getValue().substring(1, o_jsonChild.getValue().length() - 1));
														...
														net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + o_jsonChild.getName() + "(" + e_jsonValueType + ") of " + p_o_jsonElement.getName());
				...
				if (o_jsonChild.getLevel() == 0) {
					if (o_jsonChild.getName().toLowerCase().contentEquals("$id")) {
						...
						if (e_jsonValueType == JSONValueType.Object) {
																	net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + "save current element(" + this.o_currentElement.getName() + ") in temporary variable");
							
							/* save current element in temporary variable */
							JSONElement o_oldCurrentElement = this.o_currentElement;

																	net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + "start recursion for: " + o_jsonChild.getName() + " = (" + e_jsonValueType + ") for " + this.o_currentElement.getName());
							...
							if (o_jsonChild.getName().toLowerCase().contentEquals("definitions")) {
								...
								this.a_properties = this.o_currentElement;
																	...
																	net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + "end recursion for " + this.o_currentElement.getName() + ": " + o_jsonChild.getName() + " = (" + e_jsonValueType + ")");
							
																	net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + "reset current element(" + this.o_currentElement.getName() + ") from temporary variable(" + o_oldCurrentElement.getName() + ")");
				...
				if (e_jsonValueType == JSONValueType.Object) {
					...
					if (!b_skip) { /* new object */
																net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + o_jsonChild.getName() + " = (" + e_jsonValueType + ")");
						...
						if (o_jsonChild.getName().toLowerCase().contentEquals("properties")) {
							...
							this.parseJSONSchema(o_jsonChild);
																	...
																	net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + "end recursion for: " + o_jsonChild.getName() + " = (" + e_jsonValueType + ")");
						} else {
																	net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + "save current element(" + this.o_currentElement.getName() + ") in temporary variable");
							...
							if (b_handleRootItems) {
								...
								if (o_newJSONElement.getChildren().size() == 1) {
																			net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "setReference for (" + this.o_currentElement.getName() + ") with object(" + o_newJSONElement.getChildren().get(0).getName() + ")");
							...
							if (this.o_currentElement.getName().toLowerCase().contentEquals("definitions")) {
								this.a_definitions = this.o_currentElement;
															...
															net.forestany.forestj.lib.Global.ilogFiner(printIndentation() + o_jsonChild.getName() + " = " + o_jsonChild.getValue() + " (" + e_jsonValueType + ") for " + this.o_currentElement.getName());
	...
	public File jsonEncode(Object p_o_object, String p_s_jsonFile, boolean p_b_overwrite, boolean p_b_prettyPrint) throws java.io.IOException, NullPointerException, IllegalArgumentException, NoSuchFieldException, NoSuchMethodException, java.lang.reflect.InvocationTargetException, IllegalAccessException, ClassNotFoundException {
		...
		String s_json = this.jsonEncodeRecursive(this.o_schema, p_o_object, false);
												...
												if (net.forestany.forestj.lib.Global.isILevel(net.forestany.forestj.lib.Global.MASS)) net.forestany.forestj.lib.Global.ilogMass("Encoded JSON:" + this.s_lineBreak + s_json);
	...
	private String jsonEncodeRecursive(JSONElement p_o_jsonSchemaElement, Object p_o_object, boolean p_b_parentIsArray) throws NullPointerException, IllegalArgumentException, NoSuchFieldException, NoSuchMethodException, java.lang.reflect.InvocationTargetException, IllegalAccessException, ClassNotFoundException {
		...
		if (p_o_jsonSchemaElement.getType().toLowerCase().contentEquals("object")) {
			...
			if (o_object == null) {
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "casted schema-object(" + p_o_jsonSchemaElement.getName() + ")[parentName=" + s_jsonReferenceParentName + "] with schema-mapping(" + p_o_jsonSchemaElement.printMapping() + ") is null");
				...
				if ( (this.i_level == 0) || (p_b_parentIsArray) ) {
					...
					if (!net.forestany.forestj.lib.Helper.isStringEmpty(s_jsonReferenceParentName)) {
					...
					if (p_o_jsonSchemaElement.getReference().getChildren().size() < 1) {
						...
						if (p_o_jsonSchemaElement.getReference().getReference() == null) {
							...
							b_childrenIteration = false;
																	...
																	net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-object(" + p_o_jsonSchemaElement.getName() + ") with its reference(" + p_o_jsonSchemaElement.getReference().getName() + ") and schema-mapping(" + p_o_jsonSchemaElement.printMapping() + ") which has another reference with recursion");
							
							/* handle reference with recursion */
							s_json += this.jsonEncodeRecursive(p_o_jsonSchemaElement.getReference(), o_object, false);
						}
					} else {
																net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-object(" + p_o_jsonSchemaElement.getName() + ") with its reference(" + p_o_jsonSchemaElement.getReference().getName() + ") which has children");
				...
				if (b_childrenIteration) {
					...
					for (JSONElement o_jsonElement : p_o_jsonSchemaElement.getChildren()) {
															net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "encode schema-object's(" + p_o_jsonSchemaElement.getName() + ") child(" + o_jsonElement.getName() + ") with schema-mapping(" + p_o_jsonSchemaElement.printMapping() + ") with recursion");
			...
			if (p_o_jsonSchemaElement.getReference() != null) {
				if (p_o_jsonSchemaElement.getReference().getChildren().size() < 1) {
					throw new IllegalArgumentException("Reference[" + p_o_jsonSchemaElement.getReference().getName() + "] of schema-element[" + p_o_jsonSchemaElement.getName() + "] with schema-type[" + p_o_jsonSchemaElement.getType() + "] must have at least one child");
														...
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-array(" + p_o_jsonSchemaElement.getName() + ")[parentName=" + s_jsonReferenceParentName + "] with its reference(" + p_o_jsonSchemaElement.getReference().getName() + ") which has children");
			} else {
				if (p_o_jsonSchemaElement.getChildren().size() != 1) {
				...
				if (!p_o_jsonSchemaElement.getChildren().get(0).getName().toLowerCase().contentEquals("items")) {
					throw new IllegalArgumentException("Schema-element[" + p_o_jsonSchemaElement.getName() + "] with schema-type[" + p_o_jsonSchemaElement.getName() + "] must have one child with name[items]");
														...
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-array(" + p_o_jsonSchemaElement.getName() + ")[parentName=" + s_jsonReferenceParentName + "] with child(items) which has children");
			}
			
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-array(" + p_o_jsonSchemaElement.getName() + ") - and cast object with p_o_object(" + p_o_object.getClass().getTypeName() + ") and schema-mapping(" + p_o_jsonSchemaElement.printMapping() + "), castonly if java.util.List=" + (p_o_object instanceof java.util.List));
			...
			if ( (o_object == null) && (p_o_jsonSchemaElement.getChildren().get(0).getPrimitiveArray()) ) {
				...
				for (int i = 0; i < a_objects.size(); i++) {
					...
					if (a_objects.get(i) == null) {
																net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "encode schema-array(" + p_o_jsonSchemaElement.getName() + ") with array element(null) and schema-mapping(" + p_o_jsonSchemaElement.printMapping() + ")");
						
						s_json += this.printIndentation() + "null," + this.s_lineBreak;
					} else if (b_primitiveArray) {
																net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "encode schema-array(" + p_o_jsonSchemaElement.getName() + ") with primitive array element(#" + (i + 1) + ") and schema-mapping(" + p_o_jsonSchemaElement.printMapping() + ")");
						
						/* add primitive array element value to JSON */
						s_json += this.printIndentation() + a_objects.get(i) + "," + this.s_lineBreak;
					} else {
																net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "encode schema-array(" + p_o_jsonSchemaElement.getName() + ") with array element(" + a_objects.get(i).getClass().getTypeName() + ") and schema-mapping(" + p_o_jsonSchemaElement.printMapping() + ") with recursion");
			...
			if (!p_o_jsonSchemaElement.getName().toLowerCase().contentEquals("items")) {
				...
				o_object = this.castObject(p_o_jsonSchemaElement, p_o_object);
													...
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "encode schema-property(" + p_o_jsonSchemaElement.getName() + "), cast string from object with p_o_object(" + p_o_object.getClass().getTypeName() + ") and schema-type(" + p_o_jsonSchemaElement.getType() + ")");
	...
	private Object castObject(JSONElement p_o_jsonElement, Object p_o_object, boolean b_castOnly) throws NoSuchFieldException, NoSuchMethodException, java.lang.reflect.InvocationTargetException, IllegalAccessException, ClassNotFoundException {
		...
		if (b_castOnly) {
			...
			if (!net.forestany.forestj.lib.Helper.isStringEmpty(p_o_jsonElement.getMapping())) {
	...
	public boolean validateAgainstSchema(String p_s_jsonFile) throws NullPointerException, IllegalArgumentException, java.io.IOException, java.text.ParseException, java.time.DateTimeException {
		...
		File o_file = new File(p_s_jsonFile, false);
												...
												net.forestany.forestj.lib.Global.ilogFiner("read all lines from json file '" + p_s_jsonFile + "'");
	...
	public boolean validateAgainstSchema(java.util.List<String> p_a_jsonLines) throws NullPointerException, IllegalArgumentException, java.text.ParseException, java.time.DateTimeException {
		...
		if (this.o_schema == null) {
			throw new NullPointerException("Cannot decode data. Schema is null.");
												...
												net.forestany.forestj.lib.Global.ilogFiner("read all lines: '" + p_a_jsonLines.size() + "'");
	    ...
	    this.validateJSON(s_json);
	   											...
	   											net.forestany.forestj.lib.Global.ilogFinest("validated json content lines");
	    ...
	    this.parseJSON(s_json);
											    ...
											    net.forestany.forestj.lib.Global.ilogFinest("parsed json content lines");
											    
												if (net.forestany.forestj.lib.Global.isILevel(net.forestany.forestj.lib.Global.MASS)) net.forestany.forestj.lib.Global.ilogMass("JSON-Schema:" + net.forestany.forestj.lib.io.File.NEWLINE + this.o_schema);
											    if (net.forestany.forestj.lib.Global.isILevel(net.forestany.forestj.lib.Global.MASS)) net.forestany.forestj.lib.Global.ilogMass("JSON-Root-Element:" + net.forestany.forestj.lib.io.File.NEWLINE + this.o_root);
	...
	private boolean validateAgainstSchemaRecursive(JSONElement p_o_jsonDataElement, JSONElement p_o_jsonSchemaElement) throws NullPointerException, IllegalArgumentException, java.text.ParseException, java.time.DateTimeException {
		...
		if (p_o_jsonSchemaElement.getType().toLowerCase().contentEquals("object")) {
			...
			if (p_o_jsonDataElement.getChildren().size() > 0) {
				...
				if ( (p_o_jsonSchemaElement.getReference() != null) && (p_o_jsonSchemaElement.getReference().getType().toLowerCase().contentEquals("object")) ) {
					s_objectType = p_o_jsonSchemaElement.getReference().getMappingClass();
														...
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_jsonSchemaElement.getName() + ": create new schema-object instance with mapping[" + p_o_jsonSchemaElement.printMapping() + "] and type[" + s_objectType + "]");
					...
					if (p_o_jsonSchemaElement.getReference().getChildren().size() < 1) {
						...
						if (p_o_jsonSchemaElement.getReference().getReference() == null) {
							...
							if (p_o_jsonSchemaElement.getName().contentEquals(p_o_jsonDataElement.getName())) {
																		net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_jsonSchemaElement.getName() + ": get schema-object[" + p_o_jsonSchemaElement.getMappingClass() + "] with reference[" + p_o_jsonSchemaElement.getReference().getName() + "]");
								...
								b_return = this.validateAgainstSchemaRecursive(p_o_jsonDataElement.getChildren().get(0), p_o_jsonSchemaElement.getReference());
																...
																net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "update current schema-element[" + p_o_jsonSchemaElement.getName() + "](" + p_o_jsonSchemaElement.printMapping() + ") with reference[" + p_o_jsonSchemaElement.getReference().getName() + "](" + p_o_jsonSchemaElement.getReference().printMapping() + ")");
				...
				/* execute children iteration */
				if (b_childrenIteration) {
															net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_jsonSchemaElement.getName() + ": get schema-object with type[" + p_o_jsonSchemaElement.getMappingClass() + "] and children[" + p_o_jsonSchemaElement.getChildren().size() + "]");
			...
			if (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_jsonDataElement.getValue())) { /* we have multiple minor objects for current array */
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_jsonSchemaElement.getName() + ": get schema-array with mapping[" + p_o_jsonSchemaElement.printMapping() + "]");
				...
				if (p_o_jsonDataElement.getChildren().size() > 0) { /* if we have objects to the new array */
					...
					for (int i = 0; i < p_o_jsonDataElement.getChildren().size(); i++) {
						...
						this.i_level--;
														...
														net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_jsonSchemaElement.getName() + ": handle array value = " + p_o_jsonDataElement.getValue() + " - mapping[" + p_o_jsonSchemaElement.printMapping() + "]");
				
				/* set array with values if we have any values */
				if ( (!net.forestany.forestj.lib.Helper.isStringEmpty(p_o_jsonDataElement.getValue())) && (!p_o_jsonDataElement.getValue().contentEquals("[]")) ) {
					...
					for (String s_value : a_values) {
						...
						Object o_returnObject = this.castObjectFromString(s_value, p_o_jsonSchemaElement.getType());
																...
																net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "add value[" + ( (o_returnObject == null) ? "null" : o_returnObject ) + "] to list of " + p_o_jsonDataElement.getName() + ", type[" + ( (o_returnObject == null) ? "null" : o_returnObject.getClass().getTypeName() ) + "]");
					}
					...
					throw new NullPointerException("'" + p_o_jsonSchemaElement.getName() + "' of '" + s_requiredProperty + "' is required, but array has no values");
													...
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_jsonSchemaElement.getName() + ": handle value = " + p_o_jsonDataElement.getValue() + " - mapping[" + p_o_jsonSchemaElement.printMapping() + "]");
			
			JSONValueType e_jsonValueType = this.getJSONValueType(p_o_jsonDataElement.getValue());
			
													net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_jsonSchemaElement.getName() + ": json value type = " + e_jsonValueType);
			...
			if (!p_o_jsonDataElement.getValue().contentEquals("null")) {
				if ( (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_jsonSchemaElement.getMapping())) && (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_jsonSchemaElement.getMappingClass())) ) {
					...
					try {
						...
						try {
							...
							Object o_foo = this.castObjectFromString(s_objectValue, p_o_jsonSchemaElement.getMappingClass());
												...
												net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "return " + b_return);
		return b_return;
	}
	...
	public Object jsonDecode(java.util.List<String> p_a_jsonLines) throws NullPointerException, IllegalArgumentException, NoSuchFieldException, NoSuchMethodException, java.lang.reflect.InvocationTargetException, IllegalAccessException, java.text.ParseException, java.time.DateTimeException, InstantiationException, ClassNotFoundException {
	    ...
	    o_foo = this.jsonDecodeRecursive(this.o_root, this.o_schema, o_foo);
	    										...
	    										net.forestany.forestj.lib.Global.ilogFinest("decoded json content lines");
	...
	private Object jsonDecodeRecursive(JSONElement p_o_jsonDataElement, JSONElement p_o_jsonSchemaElement, Object p_o_object) throws NullPointerException, IllegalArgumentException, NoSuchFieldException, NoSuchMethodException, java.lang.reflect.InvocationTargetException, IllegalAccessException, java.text.ParseException, java.time.DateTimeException, InstantiationException, ClassNotFoundException {
		...
		if (p_o_jsonSchemaElement.getType().toLowerCase().contentEquals("object")) {
			...
			if (p_o_jsonDataElement.getChildren().size() < 1) {
				...
				if (!net.forestany.forestj.lib.Helper.isStringEmpty(p_o_jsonSchemaElement.getMapping())) {
				...
				if (p_o_jsonSchemaElement.getReference() != null) {
					...
					if (p_o_jsonSchemaElement.getReference().getChildren().size() < 1) {
						...
						if (p_o_jsonSchemaElement.getReference().getReference() == null) {
							...
							/* check if current element in schema has data element by name, otherwise skip this element */
							if (p_o_jsonSchemaElement.getName().contentEquals(p_o_jsonDataElement.getName())) {
																		net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_jsonSchemaElement.getName() + ": get schema-object[" + o_object.getClass().getTypeName() + "] with reference[" + p_o_jsonSchemaElement.getReference().getName() + "]");
				...
				/* execute children iteration */
				if (b_childrenIteration) {
															net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_jsonSchemaElement.getName() + ": get schema-object with type[" + o_object.getClass().getTypeName() + "] and children[" + p_o_jsonSchemaElement.getChildren().size() + "]");
					...
					if ( (!(p_o_object instanceof java.util.List)) && (p_o_object != null) ) {
																net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + p_o_jsonSchemaElement.getName() + ": set schema-object[" + o_object.getClass().getTypeName() + "] to current object[" + p_o_object.getClass().getTypeName() + "] with mapping[" + s_objectMapping + "]");
			...
			if ( (p_o_jsonSchemaElement.getChildren().size() > 0) && (p_o_jsonSchemaElement.getChildren().get(0).getPrimitiveArray()) ) {
				...
				if ( (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_jsonSchemaElement.getMapping())) && (net.forestany.forestj.lib.Helper.isStringEmpty(p_o_jsonSchemaElement.getMappingClass())) ) {
					...
					} else if (!(net.forestany.forestj.lib.Helper.isStringEmpty(p_o_jsonSchemaElement.getMappingClass()))) {
				...
				if (p_o_object == null) { /* create a new object instance of json array element */
					...
					o_objectList = (Object)(new java.util.ArrayList<Object>());
															...
															net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "p_o_object(" + p_o_object.getClass().getTypeName() + ") != null, get list property " + p_o_jsonSchemaElement.getName());
					...
					for (int i = 0; i < p_o_jsonDataElement.getChildren().size(); i++) {
						...
						if (o_returnObject != null) {
							...
							java.util.List<Object> o_temp = (java.util.List<Object>)o_objectList;
																	...
																	net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + "add return value to list of " + p_o_jsonDataElement.getName() + ", type[" + o_returnObject.getClass().getTypeName() + "]");
			...
			this.setProperty(p_o_jsonSchemaElement, p_o_object, p_o_jsonDataElement.getValue());
												...
												net.forestany.forestj.lib.Global.ilogFiner(this.printIndentation() + "return " + ( (p_o_object == null) ? "null" : p_o_object.getClass().getTypeName() ) );
		return p_o_object;
	}
	...
	*/
		...
		if (this.b_usePropertyMethods) {
			...
			try {
				o_method.invoke(p_o_object, this.castObjectFromString(p_s_objectValue, p_o_jsonSchemaElement.getType()));
														...
														net.forestany.forestj.lib.Global.ilogFinest(this.printIndentation() + o_exc.getMessage() + " - with type[" + p_o_jsonSchemaElement.getType() + "] - try another cast with type[" + p_o_object.getClass().getDeclaredField(s_field).getType().getTypeName() + "]");
	...
	public enum JSONValueType {
		...
		String,
		/**
		 * Number json value type
		 */
		Number,
		/**
		 * Integer json value type
		 */
		Integer,
		/**
		 * Boolean json value type
		 */
		Boolean,
		/**
		 * Array json value type
		 */
		Array,
		/**
		 * Object json value type
		 */
		Object,
		/**
		 * Null json value type
		 */
		Null
	}
	...
	public class JSONElement {
		 ...
		 * set reference
		 * 
		 * @param p_o_value reference to other JSONElement
		 */
		 ...
		 * 
		 * @return java.util.List&lt;JSONElement&gt;
		 */
		 ...
		 * 
		 * @return java.util.List&lt;JSONRestriction&gt;
		 */
		 ...
		 * print mapping
		 * 
		 * @return mapping string of json element, mapping class or a combination of mapping and mapping class separated by ':'
		 */
