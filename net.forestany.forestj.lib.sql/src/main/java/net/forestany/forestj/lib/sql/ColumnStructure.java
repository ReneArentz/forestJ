							...
							for (String s_constraint : this.a_constraintList) {
								if (this.s_alterOperation == "CHANGE") { /* change a column */
									...
									if (s_constraint == "PRIMARY KEY") {
										...
										/* 'serial' is the old way to auto generate primary/unique values and it is not part of the SQL standard */
										//s_foo += "\"" + s_fooName + "\" serial " + s_constraint;
										s_foo += "\"" + s_fooName + "\"" + " " + this.s_columnType + " " + s_constraint + " " + "GENERATED by default AS IDENTITY";
										break;
									} else {
