 ...
 *
 * @param <T>	Class definition of current class, e.g. Test extends de.forestj.lib.io.FixedLengthRecord<Test.class>
 */
public abstract class FixedLengthRecord<T> {
		...
		for (String s_field : this.OrderBy.keySet()) {
			if (!this.fieldExists(s_field)) {
				throw new NoSuchFieldException("OrderBy[" + s_field + "] is not a field of current record class.");
												...
												if (de.forestj.lib.Global.isILevel(de.forestj.lib.Global.MASS)) de.forestj.lib.Global.ilogMass("created fixed length record object");
	}
	...
	protected <T2> T2 getFieldValue(String p_s_field) throws NoSuchFieldException, IllegalAccessException {
		p_s_field = "Field" + p_s_field;
												...
												if (de.forestj.lib.Global.isILevel(de.forestj.lib.Global.MASS)) de.forestj.lib.Global.ilogMass("get field value from '" + p_s_field + "'");
	...
	protected void setFieldValue(String p_s_field, Object o_value) throws NoSuchFieldException, IllegalAccessException {
		...
		if (o_value != null) {
													if (de.forestj.lib.Global.isILevel(de.forestj.lib.Global.MASS)) de.forestj.lib.Global.ilogMass("set field value for: '" + p_s_field + "'" + "\t\tfield type: " + this.getClass().getDeclaredField(p_s_field).getType().getTypeName() + "\t\tvalue type: " + o_value.getClass().getTypeName());
	...
	protected T readFieldsFromString(String p_s_line) throws IllegalArgumentException, ClassCastException, java.lang.reflect.InvocationTargetException, IllegalAccessException, java.lang.InstantiationException, NoSuchMethodException, ClassNotFoundException, NoSuchFieldException, IllegalStateException {
		...
		for (java.util.Map.Entry<Integer, StructureElement> o_structureElement : this.Structure.entrySet()) {
			...
			if (!de.forestj.lib.Helper.isStringEmpty(o_structureElement.getValue().getConstant())) {
            ...
            if (o_structureElement.getValue().getSubType() != null) {
				...
				if ( (!de.forestj.lib.Helper.isStringEmpty(s_value.trim())) && (!s_value.matches("^[0]+$")) ) {
															if (de.forestj.lib.Global.isILevel(de.forestj.lib.Global.MASS)) de.forestj.lib.Global.ilogMass("field '" + o_structureElement.getValue().getField() + "(" + i_position + ".." + (i_position + o_structureElement.getValue().getLength()) + ")'\t\tunique '" + (this.Unique.contains(o_structureElement.getValue().getField()) ? "yes" : "no ") + "'\t\tvalue '" + s_value + "'\t\tconverted value '" + ( (o_structureElement.getValue().getReadTranspose() == null) ? o_structureElement.getValue().getReadFPNTranspose().TransposeValue(s_value, o_structureElement.getValue().getPositionDecimalSeparator()) : o_structureElement.getValue().getReadTranspose().TransposeValue(s_value) ) + "'");
				...
				} else if ( (!de.forestj.lib.Helper.isStringEmpty(s_value.trim())) && (s_value.matches("^[0]+$")) ) { /* value has only character '0' */
					/* get class type */
					...
					if ( (s_class.contentEquals("java.util.Date")) || (s_class.contentEquals("java.time.LocalDateTime")) || (s_class.contentEquals("java.time.LocalDate")) || (s_class.contentEquals("java.time.LocalTime")) ) {
																if (de.forestj.lib.Global.isILevel(de.forestj.lib.Global.MASS)) de.forestj.lib.Global.ilogMass("field '" + o_structureElement.getValue().getField() + "(" + i_position + ".." + (i_position + o_structureElement.getValue().getLength()) + ")'\t\tunique '" + (this.Unique.contains(o_structureElement.getValue().getField()) ? "yes" : "no ") + "'\t\tvalue '" + s_value + "'\t\tconverted value 'null'");
	...
	protected String writeFieldsToString() throws NoSuchFieldException, java.lang.reflect.InvocationTargetException, ClassNotFoundException, IllegalAccessException, java.lang.InstantiationException, NoSuchMethodException {
		...
		for (java.util.Map.Entry<Integer, StructureElement> o_structureElement : this.Structure.entrySet()) {
            ...
            if (o_structureElement.getValue().getSubType() != null) {
                ...
                if (de.forestj.lib.Helper.isStringEmpty(s_field)) {
            ...
            } else if (!de.forestj.lib.Helper.isStringEmpty(o_structureElement.getValue().getConstant())) { /* check if we have a constant field */
