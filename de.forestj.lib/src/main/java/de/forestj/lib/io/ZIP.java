...
public class ZIP {
	...
	public static long getSize(String p_s_sourceLocation) throws IllegalArgumentException, java.io.IOException, IllegalStateException {
    	...
    	if (!de.forestj.lib.io.File.exists(p_s_sourceLocation)) {
		...
		try {
            ...
            while (o_zipEntry != null) {
                ...
                int i_length;
                										...
                										de.forestj.lib.Global.ilogMass("start reading file length of '" + o_zipEntry.getName() + "'");
                ...
                while ((i_length = o_zipInputStream.read(a_buffer)) > 0) {
                	...
                	l_return += i_length;
                										...
                										de.forestj.lib.Global.ilogMass("finished reading file length of '" + o_zipEntry.getName() + "'");
        ...
        try {
            ...
            while (o_zipEntry != null) {
                ...
                int i_length;
                										...
                										de.forestj.lib.Global.ilogMass("start checking file in archive '" + o_zipEntry.getName() + "'");
                ...
                while ((i_length = o_zipInputStream.read(a_buffer)) > 0) {
                	if (p_itf_delegate != null) {
                    	...
                    	p_itf_delegate.PostProgress( (double)l_sum / l_overallSum );
                										...
                										de.forestj.lib.Global.ilogMass("finished checking file in archive '" + o_zipEntry.getName() + "'");
	...
	public static void zip(String p_s_sourceLocation, String p_s_destinationLocation, ZIP.IDelegate p_itf_delegate) throws IllegalArgumentException, java.io.IOException, IllegalStateException {
        ...
        try {
            ...
            if (p_itf_delegate != null) {
				for (ListingElement o_listingElement : de.forestj.lib.io.File.listDirectory(p_s_sourceLocation, true)) {
            ...
            for (ListingElement o_listingElement : de.forestj.lib.io.File.listDirectory(p_s_sourceLocation, true)) {
            	...
            	java.util.zip.ZipEntry o_zipEntry = new java.util.zip.ZipEntry(o_listingElement.getName());
            											...
            											de.forestj.lib.Global.ilogMass("start zipping file to archive '" + o_zipEntry.getName() + "'");
                ...
                o_fileInputStream = null;
                										...
                										de.forestj.lib.Global.ilogMass("finished zipping file to archive '" + o_zipEntry.getName() + "'");
            }
        } finally {
    ...
    public static void unzip(String p_s_sourceLocation, String p_s_destinationLocation, boolean p_b_createDestinationPath, boolean p_b_deleteSourceLocation, ZIP.IDelegate p_itf_delegate) throws IllegalArgumentException, java.io.IOException, IllegalStateException {
    	...
    	if (!de.forestj.lib.io.File.folderExists(p_s_destinationLocation)) {
    		...
    		if (!p_b_createDestinationPath) {
    			...
    			de.forestj.lib.io.File.createDirectory(p_s_destinationLocation);
    		}
    	}
    	    	
    	/* check if destination location is a directory */
		if (!de.forestj.lib.io.File.isDirectory(p_s_destinationLocation)) {
		...
		try {
            ...
            while (o_zipEntry != null) {
            	/* get zip entry path */
            	String s_pathParentFolder = java.nio.file.Paths.get(p_s_destinationLocation + de.forestj.lib.io.File.DIR + o_zipEntry.getName()).toFile().getParent();
            	
            	/* check if path to parent folder ends with directory separator */
            	if (!s_pathParentFolder.endsWith(de.forestj.lib.io.File.DIR)) {
            		s_pathParentFolder += de.forestj.lib.io.File.DIR;
            	}
            	
            	/* check if path exists */
				if (!de.forestj.lib.io.File.folderExists(s_pathParentFolder)) {
					/* create path */
					de.forestj.lib.io.File.createDirectory(s_pathParentFolder);
				}
            	
                /* check if destination file can be created */
            	java.nio.file.Path o_path = java.nio.file.Files.createFile(java.nio.file.Paths.get(p_s_destinationLocation + de.forestj.lib.io.File.DIR + o_zipEntry.getName()));
				
            	/* check if destination file exists */
				if (!de.forestj.lib.io.File.exists(p_s_destinationLocation + de.forestj.lib.io.File.DIR + o_zipEntry.getName())) {
					throw new java.io.IOException("Could not create file[" + p_s_destinationLocation + de.forestj.lib.io.File.DIR + o_zipEntry.getName() + "]");
				}
                ...
                int i_length;
                										...
                										de.forestj.lib.Global.ilogMass("start unzipping file from archive '" + o_zipEntry.getName() + "'");
                ...
                while ((i_length = o_zipInputStream.read(a_buffer)) > 0) {
                    ...
                    if (p_itf_delegate != null) {
                    	...
                    	p_itf_delegate.PostProgress( (double)l_sum / l_overallSum );
                										...
                										de.forestj.lib.Global.ilogMass("finished unzipping file from archive '" + o_zipEntry.getName() + "'");
        ...
        if (p_b_deleteSourceLocation) {
        	...
        	de.forestj.lib.io.File.deleteFile(p_s_sourceLocation);
        }
    }
}
