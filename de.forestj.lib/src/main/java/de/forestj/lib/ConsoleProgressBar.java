...
public class ConsoleProgressBar {
	...
	public ConsoleProgressBar(long p_l_animationInterval, int p_i_blockLength, int p_i_marqueeLength, int p_i_marqueeInterval) {
		...
		this.i_marqueeInterval = p_i_marqueeInterval;
												...
												de.forestj.lib.Global.ilogConfig("animationInterval: '" + this.l_animationInterval + "'");
												de.forestj.lib.Global.ilogConfig("blockLength: '" + this.i_blockLength + "'");
												de.forestj.lib.Global.ilogConfig("marqueeLength: '" + this.i_marqueeLength + "'");
												de.forestj.lib.Global.ilogConfig("marqueeInterval: '" + this.i_marqueeInterval + "'");
	}
	...
	public void init(String p_s_startText, String p_s_doneText, String p_s_marqueeText) throws IllegalStateException {
		...
		this.o_progressBarTimerHandler.setMarqueeText(p_s_marqueeText);
												...
												de.forestj.lib.Global.ilogConfig("startText: '" + p_s_startText + "'");
												de.forestj.lib.Global.ilogConfig("doneText: '" + p_s_doneText + "'");
												de.forestj.lib.Global.ilogConfig("marqueeText: '" + p_s_marqueeText + "'");
		...
		this.o_timer.schedule(this.o_progressBarTimerHandler, 0, this.l_animationInterval);
												...
												de.forestj.lib.Global.ilogFinest("ConsoleProgressBar timer started");
												de.forestj.lib.Global.ilogFinest("ConsoleProgressBar initialised");
	}
	...
	public void report(double p_d_value) {
		if (this.o_progressBarTimerHandler != null) {
			this.o_progressBarTimerHandler.setProgress(Math.max(0, Math.min(1, p_d_value)));
													if (de.forestj.lib.Global.isILevel(de.forestj.lib.Global.MASS)) de.forestj.lib.Global.ilogMass("Reported progress: '"  + Math.max(0, Math.min(1, p_d_value)) + "'");
		}
	}
	...
	public void close() {
		...
		if (this.o_timer != null) {
			this.o_timer.cancel();
			this.o_timer.purge();
													de.forestj.lib.Global.ilogFinest("ConsoleProgressBar timer stopped");
		}
		
												de.forestj.lib.Global.ilogFinest("ConsoleProgressBar closed");
	}
	...
	private class ProgressBarTimerHandler extends java.util.TimerTask {
		...
		public void close() {
			...
			System.out.print(this.s_doneText + de.forestj.lib.io.File.NEWLINE);
		}
