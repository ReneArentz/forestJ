			...
			for (java.lang.reflect.Field o_field : this.getClass().getDeclaredFields()) {
				...
				try {
					if ( (o_field.get(this) instanceof ShipTo) || (o_field.get(this) instanceof ShipItemInfo) ) {
						...
						s_foo += o_field.getName() + " = " + de.forestj.lib.Helper.printArrayList( java.util.Arrays.asList( (java.math.BigDecimal[])o_field.get(this) ) ) + "|";
					} else {
						s_foo += o_field.getName() + " = " + o_field.get(this).toString() + "|";
					}
				} catch (Exception o_exc) {
			...
			for (java.lang.reflect.Field o_field : this.getClass().getDeclaredFields()) {
				...
				try {
					if (o_field.get(this) instanceof java.util.List<?>) {
						...
						s_foo += o_field.getName() + " = " + de.forestj.lib.Helper.printArrayList( java.util.Arrays.asList( (Boolean[])a_foo ) ) + "|";
					} else {
						s_foo += o_field.getName() + " = " + o_field.get(this).toString() + "|";
					}
				} catch (Exception o_exc) {
			...
			for (java.lang.reflect.Field o_field : this.getClass().getDeclaredFields()) {
				...
				try {
					if (o_field.getType().getTypeName().endsWith("[]")) {
						...
						s_value = de.forestj.lib.Helper.printArrayList( java.util.Arrays.asList( (Float[])a_foo ) );
					} else {
						s_value = o_field.get(this).toString();
					}
				} catch (Exception o_exc) {
